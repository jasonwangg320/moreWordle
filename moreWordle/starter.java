...
public class starter implements InputControl, InputKeyControl{
	...
	public static boolean[] green = new boolean[5]; 
	public static boolean[] yellow = new boolean[5]; 
	...
	for (int i = 0; i<5; i++){
		if (green[i]){
			String temp = s[i]; 
			if (qp.indexOf(temp) >= 0){
				top[qp.indexOf(temp)].setColor(new Color (0,180, 0)); 
				top[qp.indexOf(temp)].fill(); 
			}
			if (al.indexOf(temp) >= 0){
				mid[al.indexOf(temp)].setColor(new Color (0,180,0)); 
				mid[al.indexOf(temp)].fill(); 
			}
			if (zm.indexOf(temp) >= 0){
				bot[zm.indexOf(temp)].setColor(new Color (0,180,0)); 
				bot[zm.indexOf(temp)].fill(); 
			}
		}
		if (yellow[i]){
			String temp = s[i]; 
			if (qp.indexOf(temp) >= 0){
				top[qp.indexOf(temp)].setColor(new Color (225,225, 0)); 
				top[qp.indexOf(temp)].fill(); 
			}
			if (al.indexOf(temp) >= 0){
				mid[al.indexOf(temp)].setColor(new Color (225,225,0)); 
				mid[al.indexOf(temp)].fill(); 
			}
			if (zm.indexOf(temp) >= 0){
				bot[zm.indexOf(temp)].setColor(new Color (225,225,0)); 
				bot[zm.indexOf(temp)].fill(); 
			}
		}
	...
	public static void checkWord(String[] array, int row){
		...
		//checking green or yellow
		for (int x = 0; x < 5; x++){
			for (int y = 0; y < 5; y++){
				if (correctWord[x].equals(array[y])){
					if (x == y)
						green[x] = true; 
					else 
						yellow[y] = true; 
		...
		for (int i = 0; i < 5; i++){
			...
			if (yellow[i]){
				display[row][i].setColor(new Color(225,225,0)); 
				display[row][i].draw(); 
			}
		}
	}
	...
	public static void clear(){
		for (int i = 0; i < qp.length(); i++){
		//	top[i].setColor(new Color(255,255,255)); 
		//	top[i].fill(); 
			top[i].undraw(); 
			topt[i].undraw(); 
		}
		for (int i = 0; i < al.length(); i++){
		//	mid[i].setColor(new Color(255,255,255)); 
		//	mid[i].fill(); 
			mid[i].undraw(); 
			midt[i].undraw(); 
		}
		for (int i = 0; i < zm.length(); i++){
		//	bot[i].setColor(new Color(255,255,255)); 
		//	bot[i].fill(); 
			bot[i].undraw(); 
			bott[i].undraw(); 
		}
	...
	public static void main(String args[]) throws FileNotFoundException, InterruptedException {
		...
		//initializing userWord to empty and green & yellow to all false
		for (int c = 0; c < 5; c++){
			userWord[c] = ""; 
			green[c] = false; 
			yellow[c] = false; 
